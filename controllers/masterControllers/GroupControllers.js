...
exports.createGroup = async (req, res) => {
  try {
    const { groupName, description, memberIds, createdBy } = req.body;

    // --- Validation ---
    if (!groupName || !memberIds || !Array.isArray(memberIds)) {
      return res.status(400).json({
        message: "Group name and an array of member IDs are required.",
      });
    }
...
exports.addMembersToGroup = async (req, res) => {
  try {
    ...
    if (!groupId || !mongoose.Types.ObjectId.isValid(groupId)) {
      return res
        .status(400)
        .json({ message: "A valid Group ID is required in the request body." });
    }
    if (!memberIds || !Array.isArray(memberIds) || memberIds.length === 0) {
      return res
        .status(400)
        .json({ message: "An array of member IDs to add is required." });
    }
    ...
    if (!group.admins.some((adminId) => adminId.equals(requesterId))) {
      return res
        .status(403)
        .json({ message: "Unauthorized: Only group admins can add members." });
    }
...
exports.getGroupMessages = async (req, res) => {
  try {
    const { groupId, page = 1, limit = 50, requesterId } = req.body;
    // --- Validation ---
    if (!groupId || !mongoose.Types.ObjectId.isValid(groupId)) {
...
exports.getUserGroups = async (req, res) => {
  try {
    ...
    if (!userId) {
      return res
        .status(400)
        .json({ message: "User ID not found. Authentication error." });
    }
...
exports.removeMemberFromGroup = async (req, res) => {
  try {
    const { groupId, memberIdToRemove, requesterId } = req.body;
    ...
    if (
      ...
      return res.status(400).json({
        message: "Valid Group ID and Member ID to remove are required.",
      });
    }
    ...
    if (!group.admins.some((adminId) => adminId.equals(requesterId))) {
      return res.status(403).json({
        message: "Unauthorized: Only group admins can remove members.",
      });
    }

    // 2. Safety Check: Prevent removing the last admin.
    const isRemovingAdmin = group.admins.some((adminId) =>
      adminId.equals(memberIdToRemove),
    );
    if (isRemovingAdmin && group.admins.length === 1) {
      return res.status(400).json({
        message:
          "Cannot remove the only admin from the group. Assign a new admin first.",
      });
    }
...
exports.makeMemberAdmin = async (req, res) => {
  try {
    const { groupId, memberIdToPromote, requesterId } = req.body;
    ...
    if (
      ...
      !requesterId ||
      !mongoose.Types.ObjectId.isValid(groupId) ||
      !mongoose.Types.ObjectId.isValid(memberIdToPromote)
    ) {
      return res.status(400).json({
        message: "Valid Group ID and Member ID to promote are required.",
      });
    }
    ...
    if (!group.admins.some((adminId) => adminId.equals(requesterId))) {
      return res.status(403).json({
        message: "Unauthorized: Only group admins can promote other members.",
      });
    }
    ...
    if (!group.members.some((memberId) => memberId.equals(memberIdToPromote))) {
      return res.status(400).json({
        message:
          "This user is not a member of the group and cannot be made an admin.",
      });
    }
    ...
    if (group.admins.some((adminId) => adminId.equals(memberIdToPromote))) {
      return res
        .status(200)
        .json({ message: "This member is already an admin.", group });
    }
